<template>
	<div id="app" class="contain">
		<GameStateStart v-if="uiState === 'start'">
			<h2>Which hooman would you like to be?</h2>
			...
			<button @click="selectCharacter">Select Your Hooman Form</button>
		</GameStateStart>
		<section v-else-if="uiState === 'characterChosen'">
			...
			<div class="zombietalk">
				<p v-for="character in shuffle(characterChoices)" :key="character">
					<button @click="selectQuestion(character)">
...
export default {
	...
	methods: {
		selectCharacter() {
			this.$store.commit('selectCharacter', this.characterInput);
			this.$store.commit('updateUIState', 'characterChosen');
		},
		selectQuestion(character) {
			this.$store.commit('selectQuestion', character);
		},
		shuffle(array) {
...
body {
	...
	background: url('./assets/background.svg') no-repeat center center scroll, #29abe2;
}
